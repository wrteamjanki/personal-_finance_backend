...
async def create_income(entry: IncomeCreate, db: AsyncSession = Depends(get_async_session)):
    new_income = await service.add_income(db, entry)
    return new_income

@router.get("/", response_model=List[IncomeEntry])
async def read_all_income(db: AsyncSession = Depends(get_async_session)):
    incomes = await service.get_all_income(db)
    return incomes

@router.delete("/{income_id}", response_model=dict)
async def remove_income(income_id: int, db: AsyncSession = Depends(get_async_session)):
...
async def edit_income(income_id: int, entry: IncomeUpdate,db: AsyncSession = Depends(get_async_session)):
    try:
...
async def income_categories(db: AsyncSession = Depends(get_async_session)):
