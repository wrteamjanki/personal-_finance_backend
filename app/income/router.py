...
async def create_income(entry: IncomeCreate, token: str = Depends(oauth2_scheme)):
    return await add_income(entry)

@router.get("/", response_model=List[IncomeEntry])
async def read_all_income(token: str = Depends(oauth2_scheme)):
    return await get_all_income()

@router.delete("/{income_id}", response_model=dict)
async def remove_income(income_id: int, token: str = Depends(oauth2_scheme)):
...
async def edit_income(income_id: int, entry: IncomeUpdate, token: str = Depends(oauth2_scheme)):
    try:
...
async def income_categories(token: str = Depends(oauth2_scheme)):
